    ...
    pub async fn session_investigate(
        ...
        let mut notifications = self
            .notifications_recent_checked(models::NotificationsRecentRequest {
                limit: Some(i64::from(section_limit) + 1),
                rule_id: None,
        ...
        let source_artifact_evidence_truncated =
        ...
        let artifact_evidence_combined_overflow =
            artifact_evidence_by_id.len() > section_limit as usize;
        let artifact_evidence = artifact_evidence_by_id
            .into_values()
            .take(section_limit as usize)
            .collect::<Vec<_>>();
        let artifact_evidence_truncated =
            source_artifact_evidence_truncated || artifact_evidence_combined_overflow;
        ...
        let result = models::SessionInvestigateResponse {
            session,
...
fn session_investigation_metadata(
    ...
    let graph_calls =
        u32::from(result.correlation.is_some()) + u32::from(result.graph_neighborhood.is_some());
    let log_rows = result
        .correlation
        .as_ref()
        .map_or(0, |value| value.logs.len())
        .saturating_add(result.incident_context.error_logs.len())
        .min(u32::MAX as usize) as u32;
    let evidence_rows = session_investigation_evidence_rows(result);
    let partial = !result.partial_reasons.is_empty();
    InvestigationMetadata {
        server_version: env!("CARGO_PKG_VERSION").to_string(),
        schema_version: INVESTIGATION_UI_VERSION.to_string(),
        graph_projection_status: result
            .correlation
            .as_ref()
        ...
        degraded_reasons: result
            ...
            .unwrap_or_default(),
        truncated: partial,
        truncation_reasons: result.partial_reasons.clone(),
        partial,
        partial_reasons: result.partial_reasons.clone(),
...
fn session_investigation_evidence_rows(
    result: &models::SessionInvestigateResponse,
) -> usize {
    let graph_rows = result.graph_neighborhood.as_ref().map_or(0, |graph| {
        graph
            ...
            .saturating_add(graph.evidence.len())
    });
    result
        .transcript
        .len()
        .saturating_add(graph_rows)
        .saturating_add(result.skill_events.len())
        .saturating_add(result.mcp_events.len())
        .saturating_add(result.hook_events.len())
        .saturating_add(result.artifact_evidence.len())
        .saturating_add(result.observatory.runs.len())
        .saturating_add(result.observatory.actors.len())
        .saturating_add(result.observatory.related_runs.len())
        .saturating_add(result.observatory.commits.len())
        .saturating_add(result.observatory.events.len())
        .saturating_add(result.observatory.spans.len())
        .saturating_add(result.observatory.metrics.len())
        .saturating_add(result.incident_context.error_logs.len())
        .saturating_add(result.notifications.len())
        .saturating_add(result.retention_lineage.len())
        .saturating_add(result.related_sessions.len())
        .saturating_add(result.external_references.len())
}
