...
const expectedPackageName = "@dinglebear/cortex-mcp";
...
function checkMetadata() {
  ...
  assert(
    ...
    `package binaryVersion ${packageJson.binaryVersion} must match package version ${packageJson.version}`,
  );
  ...
  if (buildInfo && buildInfo.version) {
    assert(buildInfo.version === packageJson.version, `server.json buildInfo.version must be ${packageJson.version}`);
  }

  const versionOverride = (npmPackage.environmentVariables || []).find((entry) => entry.name === "CORTEX_RMCP_VERSION");
  if (versionOverride && versionOverride.placeholder) {
    assert(versionOverride.placeholder === `v${packageJson.version}`, `CORTEX_RMCP_VERSION placeholder must be v${packageJson.version}`);
  }
