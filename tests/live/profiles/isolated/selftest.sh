...
bash -n "$root/tests/live/lib/docker.sh" "$root/tests/live/lib/wait.sh" "$root/tests/live/lib/diagnostics.sh" "$root/tests/live/runner.sh"
...
live_platform_disposition Linux | jq -e '.platform=="Linux" and .disposition=="not-authorized" and (.executable|endswith("scenario.sh"))' >/dev/null
