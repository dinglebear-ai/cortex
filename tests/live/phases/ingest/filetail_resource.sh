...
case "$mode" in
  ...
  verify)
    response="$(curl -fsS --max-time 8 "${headers[@]}" -d '{"op":"list"}' "$url/api/file-tails")"
    jq -e '.sources | type == "array"' <<<"$response" >/dev/null
    ! jq -e --arg id "$id" 'any(.sources[]?;.id==$id)' <<<"$response" >/dev/null
    ;;
  *) exit 2;;
esac
