...
async fn main() {
    ...
    let (state, auth, machine_ingest) = testing::oauth_state_with_machine_ingest(&data).await;
    let now = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap()
        .as_secs() as i64;
    ...
    auth.store
        ...
        .await
        .unwrap();
    let other_dir = data.join("foreign-key");
    fs::create_dir_all(&other_dir).unwrap();
    let (_, foreign) = testing::oauth_state_with_auth_state(&other_dir).await;
    let mut wrong_issuer = claims(&auth, "cortex:read", 900);
    wrong_issuer.iss = "https://attacker.invalid".into();
    let mut wrong_audience = claims(&auth, "cortex:read", 900);
    wrong_audience.aud = "https://other.invalid/mcp".into();
    let read = auth
        .signing_keys
        .issue_access_token(&claims(&auth, "cortex:read", 900))
        .unwrap();
    ...
    // Serve the machine-ingest routes too, with peer addresses as production
    // does, so auth qualification exercises the real routes rather than 404s.
    let app = router(state).merge(machine_ingest);
    axum::serve(
        listener,
