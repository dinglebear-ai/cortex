...
| **backuphost** | Unraid 7.x | backup sink + future cortex host | — | 2×8TB mirror (spinners) | 2.5GbE |
...
| claude/codex `.jsonl` transcripts | `cortex sessions index` / `cortex sessions add` scanner | ✅ implemented; local scanner only |
...
| **backuphost** | none | future cortex host |
...
- **cortex**: "every claude session on devhost last Tuesday between 2-4am" (structured filter + time)
...
- **claude/codex transcript metadata** — implemented; scanner imports local `.jsonl` transcripts into dedicated AI columns, scrubs known token patterns before storage, and queries via `cortex sessions` / `cortex sessions *`. OTLP attributes still remain producer-supplied metadata and should not be treated as verified identity.
...
1. **Stand up cortex container on backuphost** — bind mount on mirror pool, expose on tailscale
