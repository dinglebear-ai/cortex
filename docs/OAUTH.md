...
- **Non-loopback OAuth deployments still need `CORTEX_TOKEN` for OTLP HTTP/protobuf `/v1/logs`, `/v1/metrics`, and `/v1/traces` unless OTLP exposure is loopback-only or service auth is explicitly disabled behind an upstream auth layer.** OTLP ingest does not accept OAuth JWTs today. Logs allow 4 MiB request bodies; metrics and traces allow 8 MiB.
