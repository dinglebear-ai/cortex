...
page=json.loads((root/'contracts/rendered-session-page.schema.json').read_text()); page_generated=json.loads((root/'docs/contracts/generated/rendered-session-page.schema.json').read_text())
if page != page_generated: sys.exit('Cortex rendered session contract drift')
...
if page.get('properties',{}).get('delivery',{}).get('const') != 'polling': sys.exit('Cortex session delivery must remain explicit polling')
print('Cortex integration contract: schema drift, rendered-session polling, 3 compatibility fixtures, and redaction fixture passed')
